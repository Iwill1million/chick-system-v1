...
import { formatCurrency, statusLabels, statusColors, cn } from "@/lib/utils";
...
export default function Dashboard() {
  ...
  const getDates = () => {
    ...
    if (dateRange === "daily") fromDate.setDate(fromDate.getDate() - 1);
    else if (dateRange === "weekly") fromDate.setDate(fromDate.getDate() - 7);
    else if (dateRange === "monthly") fromDate.setMonth(fromDate.getMonth() - 1);
    else fromDate.setFullYear(fromDate.getFullYear() - 10); // 'all'
    return { from: fromDate.toISOString(), to };
  };

  const { data: summary, isLoading } = useGetFinanceSummary({ query: getDates() });
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="إجمالي الإيرادات" value={formatCurrency(summary.totalRevenue)} icon={TrendingUp} trend="+12.5%" />
        <StatCard title="المبلغ المحصل" value={formatCurrency(summary.totalCollected)} icon={Wallet} trend="+8.2%" />
        <StatCard title="إجمالي الطلبات" value={summary.totalOrders.toString()} icon={ShoppingCart} />
        <StatCard title="الطلبات المنجزة" value={summary.deliveredOrders.toString()} icon={CheckCircle} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2">
          <Card className="p-6 h-full flex flex-col">
            ...
            <div className="flex-1 min-h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={summary.agentPerformance} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e5e7eb" />
                  <XAxis dataKey="agentName" axisLine={false} tickLine={false} tick={{ fill: '#6b7280' }} dy={10} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: '#6b7280' }} dx={-10} />
                  <RechartsTooltip 
                    formatter={(val, name) => [
                      name === 'ordersCompleted' ? val : formatCurrency(val as string),
                      name === 'ordersCompleted' ? 'الطلبات المنجزة' : 'التحصيل'
                    ]}
                  />
                  <Legend wrapperStyle={{ paddingTop: '20px' }} />
                  <Bar dataKey="ordersCompleted" name="الطلبات المنجزة" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} maxBarSize={50} />
                  <Bar dataKey="totalCollected" name="التحصيل" fill="hsl(var(--accent))" radius={[4, 4, 0, 0]} maxBarSize={50} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </Card>
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              {summary.customerBalances.slice(0, 5).map((c, i) => (
                <div key={i} className="flex justify-between items-center p-3 rounded-xl hover:bg-secondary/50 transition-colors border border-transparent hover:border-border/50">
                  <div>
